...
fn collect_footnote_numbers(
    ...
    for slot in slots.values() {
        for fragment in slot.fragments() {
            if let FragmentKind::Footnotes { entries } = fragment.kind() {
                for entry in entries {
                    numbers.insert(entry.label().to_owned(), entry.number());
                }
            }
        }
    }
...
fn missing_footnote_render_error(label: &str) -> BuildError {
    BuildError::new(
        ErrorKind::Layout,
        None,
        format!("footnote reference `[^{label}]` has no render mapping"),
        "keep footnote references synchronized with parsed footnote definitions",
    )
}

fn unexpected_footnote_definition_render_error(label: &str) -> BuildError {
    let message = if label.is_empty() {
        "unexpected footnote definition end during render".to_owned()
    } else {
        format!("unexpected footnote definition `[^{label}]` during render")
    };
    BuildError::new(
        ErrorKind::Layout,
        None,
        message,
        "keep footnote definitions collected by the parser before rendering",
    )
}
...
<head>
  ...
  <style>
    ...
    [data-peitho-agenda] { overflow: hidden; padding: 0 16px 14px; display: grid; grid-template-columns: 10px minmax(0, 1fr) auto auto; column-gap: 8px; align-content: start; }
    [data-peitho-agenda-head], [data-peitho-agenda-list], [data-peitho-agenda-row] { grid-column: 1 / -1; display: grid; grid-template-columns: subgrid; }
    [data-peitho-agenda-head] { align-items: center; margin-bottom: 4px; }
    ...
    [data-peitho-agenda-hint] { grid-column: 3 / 5; text-align: center; white-space: nowrap; }
    [data-peitho-presenter="agenda-slot"] { min-height: 0; overflow: hidden; }
    [data-peitho-agenda-row] { align-items: center; min-height: 28px; padding: 6px 0; }
...
mod tests {
  ...
  <figure class="code"><slot name="code" accepts="code" arity="0..*"></slot></figure>
    ...
    #[test]
    fn breaks_true_applies_inside_footnote_bodies() {
        let rendered = render_checked_deck_with_layout(
            "---\nbreaks: true\n---\n<!-- {\"key\":\"breaks-note\"} -->\n# Title\n\nBody[^a].\n\n[^a]: first\nsecond",
            title_body_layout(),
        );
    ...
    #[test]
    fn renders_heading_footnote_reference_with_same_slide_map() {
        let rendered = render_checked_deck_with_layout(
            "<!-- {\"key\":\"heading-slide\"} -->\n# Heading[^h]\n\n[^h]: Heading note.",
            title_body_layout(),
        );
    ...
    #[test]
    fn footnote_label_text_does_not_leak_into_rendered_html() {
        let rendered = render_checked_deck_with_layout(
            "# Title\n\nBody[^raw<label>].\n\n[^raw<label>]: Safe text.",
            title_body_layout(),
        );
...
"#;
  ...
  <slot name="left" accepts="blocks" arity="1..*"></slot>
    ...
    fn presenter_index_includes_agenda_css_with_data_selectors() {
        ...
        assert!(html
            .contains(r#"[data-peitho-agenda-head] { align-items: center; margin-bottom: 4px; }"#));
        ...
        assert!(html
            .contains(r#"[data-peitho-agenda-hint] { grid-column: 3 / 5; text-align: center; white-space: nowrap; }"#));
        assert!(!html.contains(r#"[data-peitho-agenda-head-spacer]"#));
        ...
        assert!(html.contains(
            r#"[data-peitho-agenda-row] { align-items: center; min-height: 28px; padding: 6px 0; }"#
        ));
        assert!(!html.contains(r#"[data-peitho-agenda-row] { display: grid; grid-template-columns: 10px minmax(0, 1fr) auto auto; gap: 8px;"#));
  ...
  <footer class="footnotes"><slot name="footnotes" accepts="blocks" arity="0..1"></slot></footer>
    ...
    fn footnote_numbers_for_fragments(
        fragments: &[SourceFragment<ResolvedImagePath>],
    ) -> BTreeMap<String, usize> {
        let mut numbers = BTreeMap::new();
        for fragment in fragments {
